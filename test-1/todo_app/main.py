...
def edit_todo_view(request: Request, todo_id: int, session: Session = Depends(get_session)):
    todo = session.get(Todo, todo_id)
    if not todo:
        raise HTTPException(status_code=404, detail="Todo not found")
...
def update_todo(
    ...
    return RedirectResponse(url=f"/todos/{todo_id}", status_code=302)

# Added this part too

@app.post("/todos/{todo_id}/delete")
def delete_todo(request: Request, todo_id: int, session: Session = Depends(get_session)):
    todo = session.get(Todo, todo_id)
    if not todo:
        raise HTTPException(status_code=404, detail="Todo not found")
    session.delete(todo)
